...
/// https://github.com/kylef-archive/WebLinking.swift + modify
...
extension HTTPURLResponse {
    ...
    /// Find a page (modify)
    public func findLinkPage(relation: String) -> Int {
        if let link = findLink(relation: relation),
            let linkURL = URL(string: link.uri),
            let page = linkURL["page"],
            let value = Int(page)
        {
            return value
        }
